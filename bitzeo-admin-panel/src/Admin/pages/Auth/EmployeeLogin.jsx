...
export default function EmployeeLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await API.post("/admin/employee-login", { email: email.trim().toLowerCase(), password, role });
      ...
      else if (role === "read-only") navigate("/read-only-dashboard", { replace: true });
        ...
        <div className="glass-card relative overflow-hidden p-8 glow-hover">
          ...
          <div className="flex items-center justify-center gap-4 mb-8">
            ...
            <div className="text-left">
              ...
              <p className="text-bp-text-secondary mt-2 text-sm">Employee & Staff Login</p>
            </div>
          </div>
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <div className="relative group">
                <select
                  ...
                  className="w-full appearance-none px-4 py-3 bg-bp-surface/60 border border-bp-border text-bp-text rounded-xl focus:ring-2 focus:ring-bp-yellow/20 focus:border-bp-yellow/60 outline-none transition-all duration-200 cursor-pointer"
                >
                  {ROLES.map((r) => (
                    <option key={r.value} value={r.value} className="bg-bp-card">{r.label}</option>
                  ))}
                </select>
                <ChevronDown size={16} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-bp-text-muted pointer-events-none" />
              </div>
              <p className="text-xs text-bp-text-muted mt-1.5">Select your employee role to access your dashboard</p>
            </div>
            ...
            <div>
              ...
              <div className="relative group">
                ...
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required placeholder="employee@example.com"
            ...
            <div className="flex justify-end">
              <Link to="/forgot-password" className="text-sm text-bp-yellow hover:text-bp-orange transition">Forgot Password?</Link>
            </div>
            ...
            <button type="submit" disabled={isLoading}
              className="w-full py-3 px-4 rounded-xl text-white font-semibold bg-gradient-to-r from-bp-orange to-bp-yellow hover:from-bp-yellow hover:to-bp-orange disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 shadow-lg shadow-bp-orange/25 hover:shadow-bp-orange/40 flex items-center justify-center">
              {isLoading ? (
          ...
          <div className="mt-6 pt-5 border-t border-bp-border">
            <p className="text-center text-sm text-bp-text-secondary">
              Admin login? <Link to="/admin-login" className="text-bp-blue hover:text-bp-cyan font-medium transition">Admin Panel</Link>
            </p>
          </div>
        </div>
      </div>
